...
def load_bot(
    ...
    """
    Load ray in production or development mode based on the `debug` flag
    ...
    Args:
        ...
        debug (bool): Boolean flag to indicate whether to run the bot in debug mode or not.

    Returns:
        FinancialBot: An instance of the FinancialBot class.
    """

    from ray_bot import initialize

    initialize(logging_config_path=logging_config_path, env_file_path=env_file_path)

    from ray_bot import utils
    from ray_bot.langchain_bot import RayBot
...
def load_bot_dev(
    ...
    """
    Load ray in dev mode: the embedding model runs on CPU and the LLM uses mocked data.
...
@ray_bot.rest_api(keep_warm_seconds=300, loader=load_bot)
def run(**inputs):
    """
...
@ray_bot_dev.rest_api(keep_warm_seconds=300, loader=load_bot_dev)
def run_dev(**inputs):
    """
