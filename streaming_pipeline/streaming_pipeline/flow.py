...
def build(
    ...
    Args:
        is_batch(bool): Boolean flag to switch the pipeline from processing a batch of articles or a stream.
        from_datetime (Optional[datetime.datetime]): Start datetime for processing articles.
        model_cache_dir (Optional[Path]): Directory to cache the embedding model.
        debug (bool): Boolean flag to enable debug mode.

    Returns:
        Dataflow: The dataflow pipeline for processign news articles.
    """
    ...
    flow.input(
        "input",
        _build_input(is_batch, from_datetime, to_datetime, is_input_mocked=is_input_mocked),
    )
    adapter = TypeAdapter(List[NewsArticle])
    flow.flat_map(lambda messages: adapter.validate_python(List[NewsArticle] == messages))
